...
- **Production trust:** **Partially verified** — `test-social-publish-claims.mjs` and `test-publish-now-live-gate.mjs` (PR #113) prove the claim and gate behavior; `test-social-posting-safety.mjs` and the OAuth suites are extended-only and stale. **Blocker before any future activation: the scheduled-publishing test must pass on the supported authentication path** (static-token API auth was removed in PR #110; the repair is in the test-fixes PR, pending merge). **Resolved 2026-07-24:** PR #114 (merged at `0bb28a5`, current main) ported `test-scheduled-publishing.mjs` to the supported session/cookie path and it passes in CI — the blocker is lifted; the pipeline itself remains Advanced only and dormant (production `liveGatesCount: 0`).
- **Migration:** none. **Replacement:** none (dormant). **Retire condition:** n/a; Any future activation requires a separate owner-ratified product decision, intentional relaxation of the unconditional hardening-layer 403, successful provider and OAuth verification, and explicit per-channel gate enablement. Passing tests or resolving a prerequisite does not authorize activation.
