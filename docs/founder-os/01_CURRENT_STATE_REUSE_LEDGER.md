...
- **Gates:** live gate on the scheduled path, `publishReadiness`, publish claims. **The manual Publish Now path skips the live gate — `evidence/publish-now-gate-review.md` finding applies before any future activation.**
- **Decision (charter):** manual posting is the product. This pipeline is Advanced-only and stays dormant; it is not deleted.
- **Trust:** **Partially verified** — `test-social-publish-claims.mjs` in strict gate; `test-scheduled-publishing.mjs`, `test-social-posting-safety.mjs`, OAuth suites extended-only; the live-gate gap is a confirmed defect.
