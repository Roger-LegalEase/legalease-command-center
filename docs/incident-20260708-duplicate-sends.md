...
| 12:00:57 to 12:01:08 | jaime.berrios@example.com clicks unsubscribe from both duplicate copies (CSV); NO unsubscribe ledger row from this window survives | CSV + `outreachUnsubscribes` |
...
| ~14:4x | 95 synthetic attempt rows written for the 12:00 batch, ids `react-attempt-recon-20260708-...`, uniformly stamped 12:00:08.242, one per unique recipient (94 processed + 1 SendGrid drop, dcalmesejr@example.com). No server-side trail: ran as a local CLI against Supabase | attempt ids and stamps |
...
4. star@example.com received 8 copies and all were delivered: manual hold plus apology, or leave in cadence?
