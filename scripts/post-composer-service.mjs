...
export function buildPostComposerContract(state, actor, postId, now = new Date().toISOString()) {
  ...
  const compactAsset = (value) => value?.value ? { name:value.value.name || "Selected asset", available:true } : { name:null, available:false, issue:value?.availability?.reason || "unavailable" };
  return {
    ok:true, generatedAt:now, post:{ id:draft.postId, href:draft.href, title:text(draft.sharedContent?.headline?.value || "Post", 180) },
    version:draft.version, fields:fieldValues(draft),
    creative:{ template:compactAsset(draft.creative?.template), logo:compactAsset(draft.creative?.logo), wilma:compactAsset(draft.creative?.wilma), background:compactAsset(draft.creative?.background), disclaimer:compactAsset(draft.creative?.disclaimers?.values?.[0]), availability:draft.creative?.availability?.key || "unavailable" },
    ...
    availability:draft.availability, capabilities:{ reads:true, edits:editable && draft.version !== null, editReason:draft.version === null ? "This Post cannot be safely saved because its current version is unavailable." : null, mutatesSource:false, schedules:false, approves:false, publishes:false, regenerates:false },
