...
export function buildPostComposerContract(state, actor, postId, now = new Date().toISOString()) {
  ...
  return {
    ...
    availability:draft.availability, capabilities:{ reads:true, edits:editable && draft.version !== null, creative:editable && draft.version !== null && creativeCatalog.availability.key !== "unavailable", variants:editable && draft.version !== null && channelProjection.availability.key !== "unavailable", editReason:draft.version === null ? "This Post cannot be safely saved because its current version is unavailable." : null, mutatesSource:false, schedules:editable && draft.version !== null, approves:reviewPlan.approval?.approveAction?.available === true && draft.version !== null, requestsChanges:reviewPlan.availability?.key !== "unavailable" && draft.version !== null, publishes:false, regenerates:editable && draft.version !== null && reviewPlan.regeneration?.state?.key !== "in_progress" },
