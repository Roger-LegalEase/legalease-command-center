...
assert.match(server, /class="nav-top-link" href="#decisions" data-nav-section="queue"/, "Operator nav should expose Queue (the company decisions stack).");
assert.match(server, /class="nav-top-link" href="#campaigns" data-nav-section="campaigns"/, "Operator nav should expose Campaigns directly.");
assert.match(server, /class="nav-top-link" href="#queue" data-nav-section="review-desk"/, "Operator nav should expose the Review Desk directly.");
assert.match(server, /class="nav-top-link" href="#reports" data-nav-section="reports"/, "Operator nav should expose Reports directly.");
assert.match(server, /class="nav-top-link" href="#more" data-nav-section="more"/, "Operator nav should expose More for everything else.");
assert.equal((server.match(/data-nav-section="/g) || []).length, 6, "Top navigation should have exactly the six operator-mode items (Phase N).");
