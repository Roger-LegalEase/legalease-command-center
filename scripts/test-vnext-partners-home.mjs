...
assert.doesNotMatch(JSON.stringify(view), /Hidden Example|partner-hidden|internalStage/i);
assert.ok(Array.isArray(view.relationships.items));
assert.ok(Object.hasOwn(view.relationships.items[0], "primaryContact"));
assert.ok(Object.hasOwn(view.relationships.items[0], "email"));
