...
export function requiredCapabilitiesForEndpoint(method = "GET", pathname = "/", input = {}) {
  ...
  if (verb === "GET" && path === "/api/ui/social/weekly") return ["read_internal"];
  if (verb === "POST" && path === "/api/ui/social/weekly/export") return ["read_internal"];
  ...
  if (verb === "GET" && path === "/api/ui/company-health") return ["read_internal"];
  if (verb === "GET" && path === "/api/ui/automation-control-center") return ["read_sensitive"];
