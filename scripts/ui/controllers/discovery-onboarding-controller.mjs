  ...
  return `(() => {
    ...
    async function save(payload, trigger) {
      ...
      try {
        ...
        if (payload.intent === "defer") { announce("Onboarding skipped for now."); close(); return; }
        openAction(result.action, trigger);
      } catch (error) { announce(error.message || "Your choice could not be saved. Nothing changed."); }
    ...
    document.addEventListener("vnext:open-onboarding", (event) => { opener = event.detail?.returnTarget || document.activeElement; if (root()) { root().hidden = false; bind(); card()?.focus(); } });
