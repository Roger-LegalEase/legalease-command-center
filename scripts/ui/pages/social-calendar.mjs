...
export function renderSocialCalendarPostCard(item) {
...
export function renderSocialCalendarPage(contract = {}) {
  ...
  return `<section class="social-calendar" data-social-calendar data-calendar-active-view="month"><header><p class="eyebrow">Social</p><h1>Calendar</h1><div role="group" aria-label="Calendar view"><button type="button" aria-pressed="true" data-calendar-view="month">Month</button><button type="button" aria-pressed="false" data-calendar-view="week">Week</button></div><label>Channel<select data-calendar-channel><option value="all">All channels</option><option value="linkedin">LinkedIn</option><option value="instagram">Instagram</option><option value="facebook">Facebook</option><option value="x">X</option><option value="threads">Threads</option></select></label><p class="social-calendar-status" data-calendar-view-status role="status">Month view · All channels</p></header><div class="social-calendar-layout"><section aria-labelledby="scheduled-title"><h2 id="scheduled-title">Scheduled Posts</h2><div class="social-calendar-grid" data-calendar-grid>${contract.items.length ? contract.items.map(renderSocialCalendarPostCard).join("") : '<p data-calendar-empty>No Posts are scheduled in this view.</p>'}</div></section><aside aria-labelledby="unscheduled-title"><h2 id="unscheduled-title">Unscheduled</h2>${contract.unscheduled.length ? contract.unscheduled.map(renderSocialCalendarPostCard).join("") : "<p>No unscheduled Posts.</p>"}</aside></div></section>`;
}

export function bindSocialCalendarPage() {
...
export function socialCalendarBrowserSource() { return `(${bindSocialCalendarPage.toString()})();`; }
