...
export function renderFilesHome(payload = {}) {
  ...
  const rows = items.map((item) => `<li class="files-row" data-file-id="${escapeAttribute(item.id)}">
    ...
    <div class="files-row-actions"><button type="button" class="files-star" data-file-star="${escapeAttribute(item.id)}" aria-pressed="${item.starred ? "true" : "false"}" aria-label="${item.starred ? "Remove from Starred" : "Add to Starred"}: ${escapeAttribute(item.name || "file")}">${item.starred ? "★" : "☆"}</button><select data-file-move="${escapeAttribute(item.id)}" aria-label="Move ${escapeAttribute(item.name || "file")} to collection"><option value="">Move…</option>${(payload.navigation?.collections || []).map((entry) => `<option value="${escapeAttribute(entry.key)}"${item.collection === entry.key ? " selected" : ""}>${escapeHtml(entry.label)}</option>`).join("")}</select></div>
  </li>`).join("");
