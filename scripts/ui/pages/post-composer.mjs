...
function composerClient(loadingHtml) {
  ...
  async function reviewAction(kind) {
    ...
    try {
      ...
      if (message) message.textContent = kind === "approve" ? "Approved. Nothing was scheduled or published." : kind === "request-changes" ? "Changes requested." : "New image ready for review; it is not approved."; await load(true);
  ...
  async function saveSchedule() {
    ...
    try {
      ...
      const body = await response.json(); if (!response.ok || !body.ok) throw new Error(body.message || "The Post could not be moved."); if (message) message.textContent = "Schedule saved. Nothing was published."; await load(true);
    } catch (error) { if (message) message.textContent = error.message; }
  }
  async function saveVariants(confirmed) {
    ...
    try {
      ...
      if (message) message.textContent = "Channels saved."; await load(true);
    } catch (error) { if (message) message.textContent = error.message; }
  }
