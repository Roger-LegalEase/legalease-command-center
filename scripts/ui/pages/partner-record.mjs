...
export function partnerRecordBrowserSource() {
  ...
  return `(() => { "use strict";
    ...
    function activityDialog(returnTarget){ const root=document.createElement("div"); root.className="partner-activity-dialog"; root.innerHTML='<section role="dialog" aria-modal="true" aria-labelledby="partner-activity-title"><h2 id="partner-activity-title">Log activity</h2><form><label>Activity type<select name="type"><option value="note_added">Note added</option><option value="meeting_completed">Meeting completed</option><option value="reply_recorded">Reply recorded</option><option value="outreach_recorded">Outreach recorded</option></select></label><label>Summary<textarea name="summary" maxlength="500" required></textarea></label><div><button type="submit">Save activity</button><button type="button" data-cancel>Cancel</button></div></form></section>'; const close=()=>{root.remove();returnTarget?.focus();}; root.querySelector("[data-cancel]").addEventListener("click",close); root.querySelector("form").addEventListener("submit",async event=>{event.preventDefault();const data=new FormData(event.currentTarget);try{await post("/api/ui/partners/"+encodeURIComponent(payload.partnerId)+"/activity",{type:data.get("type"),summary:data.get("summary")});close();await load({force:true});announce("Partner activity saved.");}catch(error){announce(error.message);}}); document.body.append(root); root.querySelector("textarea").focus(); }
    async function runAction(action,button){ try{ if(action==="log_activity"){activityDialog(button);return;} if(action==="create_outreach"){const result=await post("/api/ui/partners/outreach/campaign",{partnerIds:[payload.partnerId],campaignName:"Follow up with "+payload.header.name,goal:"Continue this Partner conversation safely."});if(result?.campaignHref){location.hash=(result.campaignHref+"?step=goal").slice(1);}return;} if(action==="add_file"){window.__LE_GLOBAL_CREATE?.openWorkflow("file-or-folder",{returnTarget:button});return;} if(action==="complete_next_action"){await post("/api/ui/partners/"+encodeURIComponent(payload.partnerId)+"/next-action/complete");await load({force:true});announce("Next action completed.");} }catch(error){announce(error.message);} }
