...
// 2026-07-13 ground-truth reset: same guard as create-demo-dataset.mjs — this loads
// fictional demo records. Explicit opt-in only; never against the Supabase backend.
if (process.env.ALLOW_DEMO_DATA !== "1") {
  console.error("prepare-launch-demo.mjs is disabled: it loads fictional demo records that were purged from real state on 2026-07-13. Set ALLOW_DEMO_DATA=1 to run it against a local sandbox only.");
  process.exit(1);
}
if ((process.env.STORAGE_BACKEND || "").toLowerCase() === "supabase") {
  console.error("prepare-launch-demo.mjs refuses to run with STORAGE_BACKEND=supabase.");
  process.exit(1);
}
...
const demoIdeaSeeds = [
  ["Clean Slate is infrastructure", "Explain why policy is only step one and implementation is the next phase.", "Implementation Layer", "LinkedIn, Twitter / X, Facebook", "Learn more", "no", "medium", "Clean editorial graphic showing the gap between policy and process."],
  ["Ask Wilma: Do I need a lawyer?", "Wilma explains supported self-help without making legal advice claims.", "Trust & Guidance", "Instagram, Twitter / X, Facebook", "Start with Wilma", "yes", "high", "Warm help desk scene with reserved Wilma space."],
  ...
  ["Partner campaigns need tracking", "Make the case that distribution without attribution is activity, not proof.", "Growth Discipline", "LinkedIn, Twitter / X", "Track the handoff", "no", "low", "Neo-brutalist launch gate visual."],
  ...
  ["Why RecordShield feeds Expungement.ai", "Explain the conversion loop from clarity to qualified cleanup demand.", "Conversion Proof", "LinkedIn, Twitter / X", "Measure the loop", "no", "low", "Data as physical proof blocks."],
  ...
  ["RecordShield as the front door", "Position RecordShield as the low-friction entry point into the LegalEase ecosystem.", "RecordShield", "LinkedIn, Twitter / X", "Start with clarity", "no", "low", "Minimal premium front-door metaphor."],
  ...
  ["Ask Wilma: What should I check first?", "Wilma explains the first safe step without legal advice.", "Wilma Explainer", "Instagram, Twitter / X, Facebook", "Ask Wilma", "yes", "high", "Reserved Wilma stage with blank cards."],
  ["Why active campaigns matter", "Ten partner campaigns creates distribution proof across multiple channels.", "Partner Campaign", "LinkedIn, Twitter / X", "Launch and measure", "no", "low", "Campaign energy with restrained color blocking."],
  ...
  ["RecordShield users are the signal", "A thousand users with source attribution creates a measurable acquisition funnel.", "Traction", "LinkedIn, Twitter / X", "Track users by source", "no", "low", "Data as sculpture with source channels."],
  ...
  ["No tracking means no story", "Without source attribution, LegalEase cannot prove what created demand.", "Growth Discipline", "LinkedIn, Twitter / X", "Fix attribution", "no", "low", "Sharp launch gate and missing path metaphor."],
  ["Ask Wilma: Is this legal advice?", "Wilma explains the difference between legal information and legal advice.", "Trust & Guidance", "Instagram, Facebook, Twitter / X", "Ask Wilma", "yes", "high", "Warm guide environment with clear reserved space."],
