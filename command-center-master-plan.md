...
**Snapshot/ledger collections grow unbounded — retention policy (raised during the pagination fix, June 27 2026).** Snapshot and per-tick ledger collections (`engagementGrowthSnapshots`, `codebaseHealthSnapshots`, `heartbeatRuns`, and similar) grow without bound — roughly one row per engine per tick. The Supabase read-truncation fix (paginated `readState`, commit `9541762`) makes hydration **correct at any table size**, so this is not a correctness risk. But unbounded growth eventually slows reads (every `readState` pages the whole table). Before the table grows large, add a retention policy — keep the last N rows per engine and prune older (engines already self-cap their in-state arrays, e.g. B4 keeps 90; the gap is the persisted table accumulating ids over time, plus `heartbeatRuns` which has no per-engine cap). **Not urgent; correctness is fixed.** Revisit when `leos_core_records` approaches tens of thousands of rows or `readState` latency becomes noticeable.
...
**B2 live-send blocked on DNS (Phase 0 done June 26 2026).** B2 Phase 0 (controlled outreach OS) is built and committed, but live sending is intentionally gated off and cannot be turned on until DNS is in place. Plan: From = `roger@legalease.com`, sending subdomain = `outreach.legalease.com` (Option A — DMARC-aligned on `legalease.com`, so the From org-domain matches the DKIM signing domain). Requires AWS / Route 53 access (currently in recovery). When DNS is back: (1) add SPF/DKIM/DMARC records for `outreach.legalease.com`, (2) set the outreach config with `sendingDomain` = `outreach.legalease.com` (POST /api/outreach/config), (3) create the SendGrid account + verify the domain, (4) wire `OUTREACH_LIVE_SEND` + the SendGrid key (and write the live client — Phase 0 leaves it unimplemented so a flag alone cannot send). Note: the same AWS account holds Route 53 for the pending `expungement.ai` cutover — recovering it unblocks BOTH this and that deferred decision.
...
3. **`render.yaml` `name:` fields are out of sync with the live services — do NOT run a Blueprint sync.** The Blueprint's `name:` fields still say `legalease-command-center` / `legalease-heartbeat`, but the LIVE services are the `-prod` variants. A Blueprint sync would create NEW services by the non-prod names instead of updating the live `-prod` ones. **We manage these via the manual dashboard, NOT Blueprint sync — do not run a Blueprint sync without first reconciling the `name:` fields to the `-prod` services.**
